...
class ResultPopulater(CallbackBase):
    ...
    def v2_runner_on_ok(self, result, **kwargs):
        ...
        if result.task_name == 'Gathering Facts':
            self.asset.mem = result._result['ansible_facts']['ansible_memtotal_mb'] * 1024 * 1024
            self.asset.system = f"{result._result['ansible_facts']['ansible_distribution']} {result._result['ansible_facts']['ansible_distribution_version']}"
            self.asset.mac = f"{result._result['ansible_facts']['ansible_default_ipv4']['macaddress']}"
            ...
            self.asset.model = data['ansible_product_name']
        if result.task_name == 'user_nums':
            self.asset.user_nums = int(result._result['stdout'])
